...
function processIcons(files) {
  ...
  return gulp.src(files).pipe(plumber(utils.errorHandler)).pipe(svgmin(config.plugins.svgmin)).pipe(cheerio(config.plugins.cheerio)).pipe(extReplace('.liquid')).pipe(size({
    showFiles: true,
    pretty: true
